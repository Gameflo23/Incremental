...
;(function (globalScope) {
  ...
  isDecimal = /^[-\+]*(Infinity|NaN|(J+|J\^\d+ )?(10(\^+|\{[1-9]\d*\})|\(10(\^+|\{[1-9]\d*\})\)\^[1-9]\d* )*((\d+(\.\d*)?|\d*\.\d+)?([Ee][-\+]*))*(0|\d+(\.\d*)?|\d*\.\d+))$/,
  ...
  P.plus=P.add=function (other){
    ...
    if (q.gt(Decimal.E_MAX_SAFE_INTEGER)||q.div(p).gt(Decimal.MAX_SAFE_INTEGER)){
      ...
      t=new Decimal([a+Math.log10(Math.pow(10,op0-a)+1),1]);
    }
    p=q=null;
    return t;
  };
  ...
  var d_lambertw=function (z,tol,principal){
    ...
    for (var i=0;i<100;++i){
      ...
      if (dd.eq(Decimal.ZERO)) return w; //Escape to fix https://github.com/Naruyoko/Decimal.js/issues/25
  ...
  P.lambertw=function (principal){
    ...
    if (principal){
      ...
      else return new Decimal(f_lambertw(x.sign*x.operator(0)));
    }else{
      ...
      else return new Decimal(f_lambertw(x.sign*x.operator(0),1e-10,false));
    }
  };
  ...
  P.tetrate=P.tetr=function (other,payload){
    ...
    var r=t.pow(y-f);
    var l=Decimal.NaN;
    for (var i=0,w=new Decimal(Decimal.E_MAX_SAFE_INTEGER);f!==0&&r.lt(w)&&i<100;++i){
      if (f>0){
        r=t.pow(r);
        if (l.eq(r)){
          f=0;
  ...
  P.arrow=function (arrows){
    ...
    if (t.isNaN()||other.isNaN()) return Decimal.NaN.clone();
      if (other.lt(Decimal.ZERO)) return Decimal.NaN.clone();
      if (t.eq(Decimal.ZERO)){
        if (other.eq(Decimal.ONE)) return Decimal.ZERO.clone();
        return Decimal.NaN.clone();
      }
      if (t.eq(Decimal.ONE)) return Decimal.ONE.clone();
      if (other.eq(Decimal.ZERO)) return Decimal.ONE.clone();
      if (other.eq(Decimal.ONE)) return t.clone();
      if (arrows.gt(Decimal.MAX_SAFE_INTEGER)){
        r=arrows.clone();
        r.layer++;
        return r;
      }
      ...
      if (t.gt("10{"+arrowsNum+"}"+MAX_SAFE_INTEGER)||other.gt(Decimal.MAX_SAFE_INTEGER)){
        if (t.gt("10{"+arrowsNum+"}"+MAX_SAFE_INTEGER)){
          ...
          r=new Decimal(t.operator(arrowsNum-1));
        }else{
          r=Decimal.ZERO;
        }
        var j=r.add(other);
      ...
      if (depth>=Decimal.maxOps+10){
        return new Decimal([[0,10],[arrowsNum,1]]);
      }
      var y=other.toNumber();
      var f=Math.floor(y);
  ...
  P.expansion=function (other){
    ...
    other=new Decimal(other);
    var r;
    if (Decimal.debug>=Decimal.NORMAL) console.log("{"+t+","+other+",1,2}");
    if (other.lte(Decimal.ZERO)||!other.isint()) return Decimal.NaN.clone();
    if (other.eq(Decimal.ONE)) return t.clone();
    if (!t.isint()) return Decimal.NaN.clone();
    if (t.eq(2)) return new Decimal(4);
    if (other.gt(Decimal.MAX_SAFE_INTEGER)) return Decimal.POSITIVE_INFINITY.clone();
    var f=other.toNumber()-1;
    r=t;
    for (var i=0;f!==0&&r.lt(Decimal.MAX_SAFE_INTEGER)&&i<100;++i){
      if (f>0){
        r=t.arrow(r)(t);
        --f;
      }
  ...
  Q.expansion=function (x,y){
    return new Decimal(x).expansion(y);
  };
  // All of these are from Patashu's break_eternity.js
  ...
  P.normalize=function (){
    ...
    do{
      ...
      if (x.array.length>Decimal.maxOps) x.array.splice(0,x.array.length-Decimal.maxOps);
      ...
      if (x.array.length<Decimal.maxOps&&x.array[0][0]!==0) x.array.unshift([0,10]);
  ...
  var decimalPlaces=function decimalPlaces(value,places){
    ...
    if (numDigits<100) numDigits=0; //A hack-y solution to https://github.com/Naruyoko/Decimal.js/issues/22
  ...
  P.toHyperE=function (){
    if (this.layer) throw Error(DecimalError+"Sorry, but this prototype doesn't support correct Hyper-E notation for numbers larger than 10{MSI}10");
    if (this.sign==-1) return "-"+this.abs().toHyperE();
    if (isNaN(this.array[0][1])) return "NaN";
    if (!isFinite(this.array[0][1])) return "Infinity";
    if (this.lt(Decimal.MAX_SAFE_INTEGER)) return String(this.array[0][1]);
    if (this.lt(Decimal.E_MAX_SAFE_INTEGER)) return "E"+this.array[0][1];
  ...
  Q.fromObject=function (input){
    ...
    return Decimal.fromArray(input.array,input.sign,input.layer);
    /*var x=new Decimal();
    x.array=[];
